                ...
                {project.gallery.map((g, i) => (
                  <figure key={i} className="pdetail__gallery-item">
                    <button
                      ...
                      onClick={() => setActiveFrame(g)}
                      aria-label={`Open ${g.caption || `frame ${i + 1}`} in lightbox`}
                    >
                      <img
                        src={g.src}
                        alt={g.caption || `${project.title} frame ${i + 1}`}
                        loading="lazy"
                        decoding="async"
                      />
                    ...
                    {g.caption && <figcaption className="pdetail__caption">{g.caption}</figcaption>}
                  </figure>
                ))}
              </div>
            </motion.div>
